...
const CircuitDashboard = () => {
  ...
  const fetchData = async () => {
    try {
    const res = await axios.get('https://api.jolpi.ca/ergast/f1/circuits/?format=json');

    const circuits = 
      res.data.MRData.CircuitTable.Circuits;
    setCarData(circuits);
    setLoading(false)
  } catch (error) {
...
<h1 className="text-xl font-bold mb-4">F1 Constructor Standings 2025</h1>
    ...
    <tbody>
    {carData.map((item) => (
      <tr key={item.circuitId} className="text-center border-b border-gray-700 
             bg-gray-900 hover:bg-gray-800 
             transition-colors duration-300">
      <td>{item.circuitName}</td>
      <td><a href={item.url} className='text-blue-600 hover:underline'>{item.circuitName}</a></td>
    ...
    <div className='sm:hidden space-y-4 bg-gray-900 p-4'>
      {carData.map((item) => (
        ...
        >
<a href={item.url} className='text-blue-600 hover:underline'>{item.circuitName}</a>
          <div className='flex flex-col flex-grow min-w-0'>
            <span className='font-bold text-lg flex items-center gap-2'>
              {item.position} <span className='font-normal'>{item.Location.locality}</span>
            </span>
            <span>
                Lat: {item.Location.lat}, Long: {item.Location.long}
            </span>
